...
SVMModel_1 = fitcsvm(train,y_train,'Standardize',false,'OptimizeHyperparameters',{'BoxConstraint','KernelScale','KernelFunction','PolynomialOrder'});
%training score
...
SVMModel_2 = fitcsvm(train,y_train,'Standardize',false,'OptimizeHyperparameters',{'BoxConstraint','KernelScale','KernelFunction','PolynomialOrder'});
%train score
...
SVMModel_3 = fitcsvm(train,y_train,'Standardize',false,'OptimizeHyperparameters',{'BoxConstraint','KernelScale','KernelFunction','PolynomialOrder'});
%train scores
...
disp("The error for the characteristics 1,2 and 4  with the one versus all method for the training set is:");
disp(tr_error);
val_error =sum((val_maxScore'~=y_valid))/length(y_valid);
disp("The error for the characteristics 1,2 and 4  with the one versus all method for the validation set is:");
disp(val_error);
test_error =sum((test_maxScore'~=y_test))/length(y_test);
disp("The error for the characteristics 1,2 and 4  with the one versus all method for the test set is:");
disp(test_error);
...
mean_w3=[sum(w3(:,1))/n; sum(w3(:,2))/n;sum(w3(:,3))/n];
...
Sw3=[sum(power(w3(:,1)- mean_w3(1),2))/n 0 0;0 sum(power(w3(:,2)-mean_w3(2),2))/n 0; 0 0 sum(power(w3(:,3)-mean_w3(3),2))/n];
...
g3=g_calculation(mean_w3,Sw3,P3,x);

d12=g1-g2;
d13=g1-g3;
d23=g2-g3;

D12=matlabFunction(g1-g2);
D13=matlabFunction(g1-g3);
D23=matlabFunction(g2-g3);

d12=simplify(vpa(D12));
d13=simplify(vpa(D13));
d23=simplify(vpa(D23));
disp(d12);
disp(d13);
disp(d23);
ew1=0;
...
%{
for i=1:10
    if (D12(w1(i,1),w1(i,2),w1(i,3))<0 && D13(w1(i,1),w1(i,2),w1(i,3))<0)
        ew1=ew1+1;
    end
    if (D12(w2(i,1),w2(i,2),w2(i,3))>0 && D23(w2(i,1),w2(i,2),w2(i,3))<0)
        ew2=ew2+1;
    end
end
ew1=ew1/10;

...
mean_w11=sum(w1(:,1))/n;
mean_w21=sum(w2(:,1))/n;
Sw11=sum(power(w1(:,1)-mean_w11,2))/n;
Sw21=sum(power(w2(:,1)-mean_w21,2))/n;
syms x;
g1=g_calculation(mean_w11,Sw11,P1,x);
g2=g_calculation(mean_w21,Sw21,P2,x);
dec=g2-g1;
xb=sym2poly(dec);
xb=roots(xb);
disp(xb);
ew1=0;
ew2=0;
D=@(x) g_calculation(mean_w11,Sw11,P1,x)-g_calculation(mean_w21,Sw21,P2,x);
for i=1:10
    if (D(w1(i,1))<0)
        ew1=ew1+1;
    end

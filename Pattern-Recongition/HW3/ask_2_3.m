...
P1=0.5;
P2=0.5;
mean_w1=[sum(w1(:,1))/n; sum(w1(:,2))/n];
mean_w2=[sum(w2(:,1))/n; sum(w2(:,2))/n];
Sw1=[sum(power(w1(:,1)- mean_w1(1),2))/n 0;0 sum(power(w1(:,2)-mean_w1(2),2))/n ];
Sw2=[sum(power(w2(:,1)- mean_w2(1),2))/n 0;0 sum(power(w2(:,2)-mean_w2(2),2))/n ];
...
ew2=0;
for i=1:10
    if (D(w1(i,1),w1(i,2))<0)
        ew1=ew1+1;
    end
    if (D(w2(i,1),w2(i,2))>0)
        ew2=ew2+1;
    end
end
ew1=ew1/10;
